...
#define map_gvg_items(m) (map[m].flag.gvg || map[m].flag.gvg_te || ((agit_flag || agit2_flag || agit3_flag) && (map[m].flag.gvg_castle || map[m].flag.gvg_te_castle)) || map[m].flag.allow_woe_items)
// Allow BG item usage
#define map_bg_items(m) (map[m].flag.battleground || (map[m].flag.pvp && battle_config.bg_items_on_pvp) || map[m].flag.allow_bg_items)
