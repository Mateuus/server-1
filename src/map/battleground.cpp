...
int bg_send_xy_timer_sub(DBKey key, DBData *data, va_list ap)
{
	...
	for( i = 0; i < MAX_BG_MEMBERS; i++ )
	{
		...
		if( battle_config.bg_idle_autokick && DIFF_TICK(last_tick, sd->idletime) >= battle_config.bg_idle_autokick && bg->g )
		{
			...
			bg_team_leave(sd,3);
			clif_displaymessage(sd->fd, "You has been kicked from battleground for being AFK.");
		...
		{ // Idle announces
			sd->state.bg_afk = 1;
			sprintf(output, "%s : %s is AFK, can be kicked using @reportafk", bg->g->name, sd->status.name);
