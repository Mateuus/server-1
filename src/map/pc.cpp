...
void pc_reg_received(struct map_session_data *sd)
{
	...
	// Battleground Teams
	sd->bg_team = pc_readglobalreg(sd, add_str(BATTLEGROUNDTEAM_VAR));
...
int pc_readparam(struct map_session_data* sd,int type)
{
	...
	switch(type) {
		...
		case SP_BATTLEGROUNDTEAM:val = sd->bg_team; break;
...
bool pc_setparam(struct map_session_data *sd,int type,int val)
{
	...
	case SP_COOKMASTERY:
		...
		return true;
	case SP_BATTLEGROUNDTEAM:
		if (val < 0)
			return false;
		if (sd->bg_team == val)
			return true;
		val = cap_value(val, 0, 3);
		sd->bg_team = val;
		pc_setglobalreg(sd, add_str(BATTLEGROUNDTEAM_VAR), sd->bg_team);
		return true;
	default:
