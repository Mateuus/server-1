...
static bool mob_parse_dbrow(char** str)
{
	...
	// Modifica el daño final de los monstruos.
	int32 atkmin_b = status->rhw.atk * battle_config.damage_multiplicator;
	int32 atkmax_b = status->rhw.atk2 * battle_config.damage_multiplicator;
	status->rhw.atk = atkmin_b;
	status->rhw.atk2 = atkmax_b;

	// Hit extremo
	int32 dexbonus = status->dex * battle_config.damage_multiplicator;
	status->dex = (dexbonus > 1500 ? 1500 : dexbonus);
