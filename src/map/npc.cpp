...
uint8 npc_selllist(struct map_session_data* sd, int n, unsigned short *item_list)
{
	...
	for( i = 0; i < n; i++ )
	{
		...
		char_id = MakeDWord(sd->inventory.u.items_inventory[idx].card[2],sd->inventory.u.items_inventory[idx].card[3]);

		if( char_id > 0 && sd->inventory.u.items_inventory[idx].card[0] == CARD0_CREATE )
		{
